...
const verificationLimiter = rateLimit({
  ...
  handler: (req, res) => {
    console.log(
      `Rate limit exceeded for IP: ${req.ip}, User: ${req.userId || "unknown"}`
    );
    res.status(429).json({
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const now = new Date();
      if (
        user.emailVerification?.otpExpiry &&
        user.emailVerification.otpExpiry > now
      ) {
        const timeRemaining = Math.ceil(
          (user.emailVerification.otpExpiry - now) / 1000 / 60
        );
        if (timeRemaining > 3) {
          // If more than 3 minutes remaining, don't send new OTP
          return res.status(429).json({
...
router.post(
  ...
  async (req, res) => {
    try {
      const { otp } = req.body;
      const user = await User.findById(req.userId).select(
        "+emailVerification.otp +emailVerification.otpExpiry"
      );
...
router.post("/resend-otp", verificationLimiter, auth, async (req, res) => {
  try {
    ...
    const emailSent = await sendOTPEmail(
      user.emailCredentials.senderEmail,
      otp,
      user.name
    );
    if (!emailSent) {
      return res.status(500).json({
        error:
          "Failed to resend verification email. Please check your email address and try again.",
      });
    }
  ...
  try {
    ...
    res.status(500).json({
      error:
        process.env.NODE_ENV === "development"
          ? error.message
          : "Failed to remove email credentials",
    });
  }
});
